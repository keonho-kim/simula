...
Action-catalog model responses use one JSON object with `label`, `intentHint`, and `expectedOutcome` per call. Action preview SSE uses these names as its `field` values after partial JSON parsing on the backend; the browser does not parse unfinished JSON.

Action-catalog recovery may reduce the requested per-visibility count while retaining at least
one validated action in every visibility. Accepted IDs and definitions remain unchanged. A
