...
Selecting an item switches to its column at 40% width and detail at 60% on desktop. Other columns are unmounted until All columns is selected. The view uses a 400 ms transform/opacity entrance, without resizing text or animating layout; reduced-motion users get an immediate switch.
