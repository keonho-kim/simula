...
class FakeRouter:
    ...
    async def ainvoke_structured_with_meta(self, role, prompt, schema, **kwargs):  # noqa: ANN001
        ...
        if schema is MajorEventPlanBatch:
            return (
                MajorEventPlanBatch(major_events=[]),
                FakeMeta(),
            )
        ...
        if schema is MajorEventUpdateBatch:
            return (
                MajorEventUpdateBatch(event_updates=[]),
                FakeMeta(),
            )
...
def test_runtime_subgraph_stops_before_next_round_on_no_progress() -> None:
    ...
    assert router.calls.get("BackgroundUpdateBatch", 0) == 1
    assert router.calls.get("ActorActionShell", 0) == 2
    assert router.calls.get("ActorActionNarrative", 0) == 2
    assert router.calls.get("RoundResolutionCore", 0) == 1
    assert router.calls.get("MajorEventUpdateBatch", 0) == 1
    assert router.calls.get("ActorIntentStateBatch", 0) == 1
    assert router.calls.get("RoundResolutionNarrativeBodies", 0) == 1
