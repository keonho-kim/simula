...
import { buildRunId } from "@/backend/storage/runs/run-id"
...
export class RunStore {
  ...
  async readManifest(runId: string): Promise<RunManifest> {
    const manifest = JSON.parse(await readFile(this.path(runId, "manifest.json"), "utf8")) as RunManifest
    return { ...manifest, artifactPaths: this.artifactPaths(runId) }
  }
