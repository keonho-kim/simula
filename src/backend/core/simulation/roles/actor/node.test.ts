...
function state(language: "ko" | "en" = "ko") {
  ...
  const input = createActorGraphState({ runId: "run", scenario: { ...parseScenarioDocument("---\nnum_cast: 2\n---\n출시를 논의하는 팀"), language }, settings: defaultSettings(), plannerDigest: "출시 일정 협의", actor, actors: [actor], event: { id: "event", title: "출시 회의", summary: "일정을 결정한다", status: "pending", participantIds: [] }, roundIndex: 1, roundDigest: { roundIndex: 1, preRound: { elapsedTime: "1", content: "답변을 기다리는 상황" } }, coordinatorTrace: emptyCoordinatorTrace() })
...
test("copied thought and intent retry only speech with targeted feedback", async () => {
  ...
  try {
    const output = await createActorStepNode("message", async event => { events.push(event) })(input)
...
test("similar meaning and silence remain valid; no_action makes no call", async () => {
  ...
  try {
    expect((await createActorStepNode("message", async () => {})(input)).trace?.retryCounts.message).toBe(0)
    expect((await createActorStepNode("message", async () => {})(input)).trace?.message).toBe("None")
...
test("persistent copied speech fails after three attempts without publishing it", async () => {
  ...
  try {
    await expect(createActorStepNode("message", async event => { events.push(event) })(input)).rejects.toThrow("3 invalid responses")
...
test("intent receives the preceding thought and current situation", async () => {
  ...
  try {
    await createActorStepNode("intent", async () => {})(input)
