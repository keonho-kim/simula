  ...
  return (
    <FieldGroup className="gap-4">
      <FieldSet className="rounded-lg bg-background/70 p-4 ring-1 ring-border/60">
        <FieldTitle>{settingsRoleLabel(role, t)}</FieldTitle>
...
function settingsRoleLabel(role: ModelRole, t: UiTexts): string {
  if (role === "storyBuilder") return t.settingsRoleStoryBuilder
  if (role === "planner") return t.settingsRolePlanner
  if (role === "generator") return t.settingsRoleGenerator
  if (role === "coordinator") return t.settingsRoleCoordinator
  if (role === "actor") return t.settingsRoleActor
  if (role === "observer") return t.settingsRoleObserver
  return t.settingsRoleRepair
}
