...
import { buildInterludeStageView, buildSimulationInterlude } from "@/ui/models/simulation/simulation-stage-interlude"
...
describe("buildSimulationInterlude", () => {
  ...
  test("localizes interlude role and stage labels", () => {
    const events = [
      runStarted(),
      actorsReady(),
      nodeStarted("coordinator", "Coordinator"),
    ...
    const interlude = buildSimulationInterlude(events, dictionary.ko)
    const view = buildInterludeStageView(events, dictionary.ko)

    expect(interlude?.roleLabel).toBe("진행")
    expect(interlude?.message).toBe("진행이 현재 시뮬레이션 상태를 읽고 있습니다.")
