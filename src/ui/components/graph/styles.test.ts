import { forceAtlasOptions } from "./layout/options"
...
const { collectNodeDepths, edgeAlpha, edgeColor, edgeSize, graphIntensityColor, nodeSize } = await import("@/ui/components/graph/styles")
...
describe("graph visual styles", () => {
  ...
  test("scales edges with visible weight contrast", () => {
    ...
    expect(weak).toBeGreaterThanOrEqual(3.4)
    expect(medium - weak).toBeGreaterThan(1.7)
  ...
  test("keeps edges visible across interaction intensities", () => {
    expect(edgeAlpha(0)).toBeGreaterThanOrEqual(0.34)
    expect(edgeAlpha(1)).toBeGreaterThanOrEqual(0.6)
  ...
  test("maps interaction intensity to distinct colors", () => {
    ...
    expect(edgeColor(1)).toContain("47, 111, 143")
  })
