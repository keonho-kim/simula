...
import { EDGE_TYPE, type ActorGraph, type GraphEdgeAttributes, type GraphNodeAttributes, type Position } from "@/ui/components/graph/types"
...
export function edgeSize(edge: GraphEdgeView): number {
  return Math.min(12, 1.35 + Math.sqrt(Math.max(0, edge.weight)) * 2.2)
}

export function edgeAlpha(weight: number): number {
  const level = intensityLevel(weight)
  if (level === "none") return 0.36
