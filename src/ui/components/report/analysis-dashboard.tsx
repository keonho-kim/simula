...
} from "@/ui/models/report/report-metrics-view-model"
import type { ReportAnalysisViewModel } from "@/ui/models/report/report-analysis-view-model"
import { DynamicsKpis, DynamicsSignalMap } from "@/ui/components/report/simulation-dynamics"
import { EmptyPanel } from "@/ui/components/report/presentation"
...
function TokenRangeControls({
  ...
  return (
    <section className="rounded-lg border border-border/70 bg-background p-3" aria-label={t.tokenRange}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-foreground">{t.tokenRange}</h2>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">{t.tokenRangeDescription}</p>
        </div>
        <div className="rounded-md border border-border/60 bg-muted/20 px-2.5 py-1.5 font-mono text-xs tabular-nums text-muted-foreground">
...
function TokenRangeSlider({
  ...
  return (
    <Field className="rounded-md border border-border/60 bg-muted/15 p-3">
      <div className="flex items-start justify-between gap-3">
        <FieldContent>
          <FieldLabel className="text-xs font-semibold">{label}</FieldLabel>
          <FieldDescription className="text-[10px]">
            {minLabel}: {min.toLocaleString("en-US")} / {maxLabel}: {max.toLocaleString("en-US")}
          </FieldDescription>
        </FieldContent>
        <span className="shrink-0 rounded-sm bg-background px-2 py-1 font-mono text-xs tabular-nums text-foreground ring-1 ring-border/60">
