...
const INTERLUDE_STAGE_IDS = ["planner", "generator", "actorCards", "coordinator", "observer"] as const
...
export function buildSimulationInterlude(events: RunEvent[], t?: UiTexts): SimulationInterludeState | undefined {
  ...
  return {
    ...
    roleLabel: roleLabel(signal.role, t),
...
export function buildInterludeStageView(
  ...
  const activeIndex = INTERLUDE_STAGE_IDS.findIndex((stageId) => stageId === activeStageId)
  const stages = INTERLUDE_STAGE_IDS.map((stageId, index) => {
    let status: InterludeStageStatus = "waiting"
    if (completed.has(stageId) || (activeIndex >= 0 && index < activeIndex && seen.has(stageId))) {
      status = "done"
    } else if (stageId === activeStageId) {
      status = completed.has(stageId) ? "done" : "active"
    }
    return { id: stageId, label: stageLabel(stageId, t), status }
  })
...
function latestInterludeSignal(events: RunEvent[], startIndex: number, t?: UiTexts): { role: ModelRole; stepLabel: string; message: string } {
  for (let index = events.length - 1; index >= startIndex; index -= 1) {
    ...
    if (event.type === "node.started" && ROLE_NODE_IDS.has(event.nodeId)) {
      const label = roleLabel(event.nodeId as ModelRole, t)
      return {
        role: event.nodeId as ModelRole,
        stepLabel: t?.interludeStarting ?? "Starting",
        message: roleMessage(label, t?.interludeReadingState ?? "is reading the current simulation state.", t),
      }
    }
    if (event.type === "node.completed" && ROLE_NODE_IDS.has(event.nodeId)) {
      ...
      return {
        ...
        message: roleMessage(label, t?.interludeFinishedPass ?? "finished its pass and handed off the next stage.", t),
      }
    }
  }
  return {
...
function stageLabel(stageId: InterludeStageId, t?: UiTexts): string {
  if (stageId === "actorCards") return t?.actorCards ?? "Actor Cards"
  return roleLabel(stageId, t)
}

function roleMessage(label: string, message: string, t?: UiTexts): string {
  return t?.languageKorean === "한국어" ? `${label}${message}` : `${label} ${message}`
}

function roleLabel(role: ModelRole, t?: UiTexts): string {
  ...
  if (role === "repair") return t?.roleRepair ?? "Repair"
