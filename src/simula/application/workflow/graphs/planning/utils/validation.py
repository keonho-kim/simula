...
def validate_execution_plan_frame_semantics(
    *,
    ...
    allowed_action_types = {
        item.action_type for item in execution_plan_frame.action_catalog.actions
    }
    for event in execution_plan_frame.major_events:
        ...
        if invalid_action_types:
            allowed_preview = ", ".join(sorted(allowed_action_types)) or "<empty>"
            issues.append(
                f"major event `{event.event_id}` uses action types outside catalog: {', '.join(invalid_action_types)}. "
                f"Allowed action_type values: {allowed_preview}. "
                "Replace each missing completion_action_types value with one allowed action_type."
            )
    return issues
