...
- `intent_states` must keep one unique item per `cast_id`.
...
- Each `intent_states` item must keep `goal` concrete.
- `time_advance.reason` must be non-empty and grounded in the adopted actions, background updates, or event progress from this round.
...
- `actor_facing_scenario_digest` must describe the next round for actors with current pressures and concise next-step notes.
