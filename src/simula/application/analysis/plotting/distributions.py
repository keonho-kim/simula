...
def render_distribution_overview(
    *,
    distributions: dict[str, MetricDistribution],
    run_id: str,
    output_path: Path,
) -> None:
    """Render one combined 2x2 overview plot for the major metrics."""
    ...
    try:
        for axis, metric in zip(axes.flat, metric_order, strict=True):
            _render_distribution_axis(
                axis=axis,
                distribution=distributions[metric],
            )
            axis.set_title(metric_label(metric))
        figure.suptitle(distribution_overview_title(run_id=run_id), fontsize=14)
